...
describe('Task Extraction', () => {
  function extractTasks(planContent: string): { title: string; instructions: string }[] {
    const tasks: { title: string; instructions: string }[] = [];
    const taskRegex = /### Task \d+: ([^\n]*)([\s\S]*?)(?=### Task|\n\n## |$)/g;
    let match;

    while ((match = taskRegex.exec(planContent)) !== null) {
      tasks.push({ title: match[1].trim(), instructions: match[2].trim() });
    }
...
describe('Cost Calculation', () => {
  ...
  function calculateCost(model: keyof typeof COST_PER_1M, inputTokens: number, outputTokens: number) {
    ...
    const input = (inputTokens / 1_000_000) * cost.input;
    const output = (outputTokens / 1_000_000) * cost.output;
