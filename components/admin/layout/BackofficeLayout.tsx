...
const BackofficeLayout: React.FC = () => {
  ...
  const SidebarContent = ({ isMobile = false }) => (
    <>
        ...
        {navItems.map((item) => (
          ...
          >
            ...
            <span className={`transition-all duration-300 whitespace-nowrap ${
              !isMobile && isCollapsed ? 'opacity-0 w-0 overflow-hidden ml-0' : 'opacity-100'
            }`}>
              {item.label}
            </span>
          </NavLink>
      ...
      <div className={`p-4 border-t border-neutral-800 transition-all duration-300 ${!isMobile && isCollapsed ? 'px-2' : 'p-4'}`}>
        <div className={`flex items-center mb-4 transition-all ${
          !isMobile && isCollapsed ? 'flex-col space-y-2 px-0' : 'space-x-3 px-2'
        }`}>
          ...
          <div className={`transition-all duration-300 flex-1 min-w-0 ${
            !isMobile && isCollapsed ? 'opacity-0 w-0 overflow-hidden h-0' : 'opacity-100'
          }`}>
        ...
        <button
          ...
          className={`group flex items-center justify-center rounded-xl bg-neutral-800 text-neutral-400 hover:bg-red-500/10 hover:text-red-400 transition-all w-full ${
            !isMobile && isCollapsed ? 'p-3' : 'space-x-2 px-4 py-2'
          }`}
        >
  ...
  return (
    <div className="flex h-screen bg-neutral-900 text-white font-sans overflow-hidden">
      {/* Sidebar - Desktop */}
      <aside className={`hidden lg:flex bg-black border-r border-neutral-800 flex-col transition-all duration-300 ease-in-out shadow-2xl z-20 ${
